...
export const PhotoStripFrame = forwardRef<HTMLDivElement, PhotoStripFrameProps>(
  function PhotoStripFrame({ photos, template, scale = 1 }, ref) {
    ...
    const frameWidth = isStrip  ? 200 * scale
                     : isGrid   ? 280 * scale
                     : isCinema ? 360 * scale
                     : 240 * scale;
    ...
    return (
      ...
      >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 * scale }}>
            {photos.map((photo, i) => (
              <PhotoSlot
                ...
                cornerRadius={frame.cornerRadius * scale}
        ...
        {/* Cinematic — horizontal, foto 3:4 portrait */}
        {isCinema && (
...
function PhotoSlot({
  photo, index, filter, cornerRadius, accentColor, aspectRatio,
}: {
  photo:        string;
  index:        number;
  filter:       string;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 7, fontWeight: 700, color: "white", opacity: 0.85 }}>
          {index + 1}
        </span>
      </div>
    </div>
...
function getFilterStyle(filter: string): string {
  switch (filter) {
    case "noir":     return "grayscale(1) contrast(1.15) brightness(0.9)";
    case "warm":     return "sepia(0.35) brightness(1.05) saturate(1.1)";
    case "cool":     return "hue-rotate(15deg) saturate(1.1) brightness(1.02)";
    case "vivid":    return "saturate(1.4) contrast(1.1)";
    case "fade":     return "brightness(1.08) saturate(0.75) contrast(0.9)";
    case "neon-pop": return "saturate(1.8) contrast(1.25) brightness(1.05)";
    default:         return "none";
  }
}